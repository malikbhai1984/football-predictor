...
export function extractStatistics(apiMatch) {
  ...
  const result = { shots_on_goal:0, shots_inside_box:0, corners:0, red_cards:0 };
  ...
  stats.forEach(teamStats => {
    ...
    teamStats.forEach(stat => {
      ...
      if(type.includes('shots on target')||type.includes('shot on')) result.shots_on_goal += value;
      if(type.includes('total shots')||type.includes('shot inside')||type.includes('shots inside')) result.shots_inside_box += value;
      if(type.includes('corner')) result.corners += value;
      if(type.includes('red card')||type.includes('redcard')||type.includes('red')) result.red_cards += value;
    });
  });
...
export function getLeagueFactor(league){
  const factors = { 'Premier League':4,'EPL':4,'PL':4, 'Bundesliga':6,'BL1':6, 'Serie A':1,'SERIE_A':1, 'Ligue 1':2,'FL1':2, 'La Liga':3,'Primera Division':3 };
...
export function realPredict(match){
  ...
  const intensity = minute<20 ? Math.min(40,Math.sqrt(intensityRaw)*4) : Math.min(100,Math.sqrt(intensityRaw)*6 + (redCards>0?-10:0));
  ...
  let over_05=totalGoals>=1 ? Math.min(92,90-Math.max(0,minute-60)*1.5) : Math.min(92,85-(minute*0.3)+intensity*0.1);
  let over_15=totalGoals>=2 ? Math.min(92,88-Math.max(0,minute-60)*1.8) : Math.min(92,75-(minute*0.4)+intensity*0.08);

  if(minute>80 && totalGoals===0){over_05=Math.min(over_05,65); over_15=Math.min(over_15,45);}
  ...
  return {
    ...
    btts:(homeScore>0 && awayScore>0)?85:(minute>75 && totalGoals===1)?45:totalGoals>=1?Math.min(78,58+intensity*0.15):35,
    next_goal:minute>85 && homeScore===awayScore?Math.min(82,60+intensity*0.25):minute>80?Math.min(75,55+intensity*0.15):minute<75?Math.min(92,70+(90-minute)*0.3+intensity*0.1):50,
